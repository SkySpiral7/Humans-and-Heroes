...
HtmlGenerator.skillRow = function (state, derivedValues)
{
   ...
   {
      if(derivedValues.hasText) htmlString += '<div class="col-12 col-md-3 col-lg-4 col-xl-auto">';
   ...
   else  //v4.0+
   {
      ...
      htmlString += '<select id="skillRank' + state.rowIndex + '" onChange="Main.skillSection.getRow(' + state.rowIndex + ').changeRank();">';
      htmlString += '<option';
      if ('Trained' === state.rank) htmlString += ' selected';
      htmlString += '>Trained</option>';
      htmlString += '<option';
      if ('Mastered' === state.rank) htmlString += ' selected';
      htmlString += '>Mastered</option>';
      htmlString += '</select>';
      htmlString += '</div>';
